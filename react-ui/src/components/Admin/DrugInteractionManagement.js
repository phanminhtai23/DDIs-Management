...
const DrugInteractionManagement = () => {
    ...
    useEffect(() => {
        ...
        fetchInteractions(1, ""); // Load trang đầu tiên
    }, []);
    ...
    useEffect(() => {
        ...
        searchTimeoutRef.current = setTimeout(() => {
            ...
            fetchInteractions(1, searchTerm);
        }, 500); // Delay 500ms
    ...
    const fetchInteractions = async (page = 1, search = "") => {
        try {
            // Check cache trước
            const cacheKey = `${page}-${search}`;
            const cached = pageCache.get(cacheKey);
            if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
                console.log(`📦 Dùng cache trang DDI ${page}`);
                setInteractions(cached.data.ddi);
            ...
            if (response.ddi && Array.isArray(response.ddi)) {
                ...
                console.log(
                    `💾 Đã cache trang DDI ${page} với ${response.ddi.length} items`
                );
            } else {
    ...
    const handleDelete = async () => {
        try {
            ...
            pageCache.clear();
            fetchInteractions(pagination.page, searchTerm);
        } catch (err) {
    ...
    const handlePageChange = ({ selected }) => {
        ...
        fetchInteractions(newPage, searchTerm);
    };
                        ...
                        <table className="w-full border-collapse">
                            <thead className="sticky top-0 bg-white z-10">
                                <tr className="bg-gray-200">
                                    ...
                                    <th className="border p-3 border-0 w-40">
                                        Hoạt Chất 1
                                    </th>
                                    <th className="border p-3 border-0 w-40">
                                        Hoạt Chất 2
                                    </th>
                                    <th className="border p-3 border-0 w-20">
                                        Mức Độ
                                    </th>
                                    <th className="border p-3 border-0 w-[480px]">
                                        Cảnh Báo
                                    </th>
                                    <th className="border p-3 border-0 w-28">
                                        Thao Tác
                                    </th>
                                </tr>
                            </thead>
                                ...
                                {interactions.length > 0 ? (
                                    interactions.map((interaction) => (
                                        ...
                                        >
                                            ...
                                            <td className="border p-2 w-20 whitespace-nowrap overflow-x-auto">
                                                {interaction.MucDoNghiemTrong ||
                                                    ""}
                                            </td>
                                            <td className="border p-2 w-[480px] max-h-24 overflow-y-auto overflow-x-hidden break-words">
